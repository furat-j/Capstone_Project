...
While our daily pipeline is reliable and idempotent, operating it in a production logistics business requires three concrete improvements, alongside key optimizations identified from our PySpark historical processing.
...
Fourth, analyzing the PySpark execution plan for historical analytics. When inspecting the physical plan for joining the 285,000 historical scans with reference data, Spark executed a BroadcastHashJoin instead of an expensive SortMergeJoin. Between the lookup tables, hubs contains ten rows and service_levels contains four rows. We explicitly broadcast service_levels and hubs because their memory footprint is minuscule (less than one kilobyte), allowing Spark to distribute them directly to executor memory. Broadcasting eliminates the need to shuffle and sort 285,000 history rows across cluster nodes, transforming a distributed network bottleneck into an instant local hash lookup.

Finally, evaluating the partitioned curated Parquet output. Writing the historical dataset partitioned by scan_date across 334 days produced 668 Parquet data files across the 334 date partition directories, averaging two files per date partition due to executor core parallelism. For an 18 MB dataset, 668 files is significantly more than expected, illustrating the classic distributed computing small file problem. Hundreds of tiny files averaging only 25 to 30 kilobytes each create unnecessary filesystem metadata overhead and degrade query performance on object storage. To fix this, I would apply coalesce(1) on each partition before writing to ensure exactly one properly-sized Parquet file per calendar day.
